...
class redis::main (
  ...
  # It doesn't really matter which controller will be Redis master
  $primary_redis_node            = $controller_nodes[0]
  ...
  $sentinel_confs = inline_template("<%=
hash = {}
@nodes.each do |value|
  hash[value] = { 'monitor'                 => value + ' ' + @redis_port + ' ' + @quorum,
...
hash %>")
  ...
  ceilometer_config {
    'coordination/backend_url'    : value => redis_backend_url($controller_nodes, $redis_sentinel_port, $timeout)
